...
import StripeTestTempoComponent from "@/components/stripe-test-tempo-component";

export default async function StripeTestTempoPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();

  // If user is not logged in, redirect to sign-in
  if (!user) {
    return redirect('/sign-in?redirect_to=/stripe-test-tempo');
  }
  ...
  if (hasSubscription) {
    return (
      <div className="container mx-auto py-12 px-4">
        <h1 className="text-2xl font-bold mb-6">Stripe Test Page (TEMPO Fix)</h1>
  ...
  return (
    <div className="container mx-auto py-12 px-4">
      ...
      <div className="bg-white border rounded-lg p-6 shadow-sm">
        <h2 className="text-xl font-semibold mb-4">Test Stripe Integration with TEMPO Fix</h2>
