...
-- This trigger will automatically seed a new user's profile with data based on the fictional Sterling Cooper agency from Mad Men
...
-- Create a trigger that calls this function whenever a user is added to auth.users
-- First check if the trigger already exists to avoid the "trigger already exists" error
DO $$
BEGIN
  ...
  ELSE
    -- If the trigger exists, drop it and recreate it (optional)
    -- Uncomment the following lines if you want to replace the existing trigger
    -- EXECUTE 'DROP TRIGGER on_auth_user_created ON auth.users';
    -- EXECUTE 'CREATE TRIGGER on_auth_user_created
    --   AFTER INSERT ON auth.users
    --   FOR EACH ROW EXECUTE PROCEDURE public.handle_new_user()';
    
    -- Or just log that the trigger already exists
    RAISE NOTICE 'Trigger on_auth_user_created already exists on auth.users table. Using the existing trigger.';
  END IF;
END
$$;

-- Comment explaining how to test the trigger
